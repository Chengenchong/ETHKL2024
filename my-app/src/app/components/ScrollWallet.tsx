...
interface ScrollWalletProps {
  onConnectionChange: (isConnected: boolean) => void;
}

export default function ScrollWallet({ onConnectionChange }: ScrollWalletProps) {
  const [balance, setBalance] = useState<string | null>(null);
  const [address, setAddress] = useState<string | null>(null);
  const [isConnected, setIsConnected] = useState<boolean>(false);

  useEffect(() => {
    const provider = getProvider('scroll');
    onConnectionChange(isConnected);
    // You can use the provider here to interact with the Scroll network
  }, [isConnected, onConnectionChange]);

  const handleConnect = async () => {
    const walletProvider = await connectWallet('scroll');
    if (walletProvider) {
